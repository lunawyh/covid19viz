...
class dataGrabFl(object):
    ...
    def lookForConfirmedPage(self, pdfReader):
        for page in range(7, 25):
            ...
            # locate page
            n_start = pageTxt.find('cities and counties of residence for cases in Florida residents')
            if(n_start >= 0): print('  found at page ', page) 
            else: continue
            # get time
            n_start = pageTxt.find('Data through')
            if(n_start >= 0):
                n_end = pageTxt.find('verified as of')
                s_date = pageTxt[n_start+13: n_end-1]
    ...
    def dataReadConfirmed(self, f_name):
            ...
            #print('the pdf page___________: ', pageTxt)
            # get text in the table list
            n_start = pageTxt.find('City and county')
            if(n_start < 0):
                    return ([], pdfReader)
                #return ([], pdfReader)
            n_end = pageTxt.find('The table below includes ')
            pageTxt = pageTxt[n_start + 21:n_end]
            print('llllllllllll',pageTxt )
isalpha
isdigit
            total_case = 0
            state_nam = []
            ...
            for a_char in pageTxt:
                a_chara.split(','):
                if(a_char[0].replace(' ', '').isalpha):
                    state_nam.append(a_char)
